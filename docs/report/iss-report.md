...
This report provides a comprehensive description of the proposed cryptographic simulation for the healthcare provider, St. John's Clinic. The implementation addresses the key security gaps within the clinic's data protection requirements, compliance standards, and security around sensitive data.

# 2 System description

**Assumptions are documented in [Appendix 4.1](#41-simulation-assumptions)**. **A read of this is highly encouraged**.

**Cross referencing the code in [Appendix 4.3](#43-implementation-source-code) along with the explanation of the implementation is also highly recommended**.

## 2.1 Authentication

### 2.1.1 User registration

([Appendix 4.3.1.1](#4311-registerpy)).
...
- **`phone_number`**: additional placeholder registration data.
...
Registration also performs two factor authentication, the below code generates a 2FA code, sends it to the provided phone number and verifies it against the user's inputted code, providing an additional layer of security.

Upon successful registration, the user's details, including a cryptographic salt, a unique user ID and a securely hashed password are stored.

### 2.1.2 User login

([Appendix 4.3.1.2](#4312-loginpy)).
...
- **`username`**: chosen user's username from registration.
- **`password`**: user's custom password from registration.

Appropriate validation checks are performed, and error messages are returned if necessary.

Using the Python `bcrypt` library, the entered password's is first hashed. This is compared against the stored hash in the database to confirm that the passwords match.

Two factor authentication is called again for more security. If the generated code matches the user's inputted code, then an RSA public key is generated for data transmission purposes (see [2.4](#24-data-transmission)).

### 2.1.3 Single Sign-on (SSO)

([Appendix 4.3.1.3](#4313-ssopy)).

[2.1.2](#212-user-login) considered homegrown SSO, i.e. a username and password based authentication within the system. The SSO here accounts for external identity authentication, where a third party will be responsible.

- **`username`**: user's unique identifier in the third party's SSO.
...
([Appendix 4.3.2.1.1](#43211-key_genpy)).

The function generates a random AES key for symmetric encryption and decryption. The key is 32 bytes (256 bits) in length, allowing for a good balance between security and performance.

#### 2.2.1.2 RSA

([Appendix 4.3.2.2](#4322-rsa_key_managerpy)).
...
([Appendix 4.3.2.1.2](#43212-key_storepy)).
...
From the AES HSM, it attempts to find an entry for that user if they already exist. If not, a new entry into the JSON collection is made, writing both the user's ID and their corresponding AES key.
...
The logic of this function is the same as AES key store.

However, the RSA key in bytes cannot be stored in JSON, so it needs to be converted into a PEM string. The two conversion functions perform this for each type of RSA key (public and private).
...
([Appendix 4.3.2.1.3](#43213-key_retrievepy)).

The stored AES keys need to be retrieved for crypto processes for the relevant user.

- **`user_id`**: ID of the user under which the AES key is stored under.

From the AES HSM, it tries to find the corresponding key entry that matches the given `user_id`. It then returns the AES key in a usable format.

#### 2.2.3.2 RSA

([Appendix 4.3.2.2](#4322-rsa_key_managerpy)).

The logic for RSA key retrieval is the same as for AES.

Since the RSA private key was stored as a PEM string (see [2.2.2.2](#2222-rsa)), it needs to be converted back into a usable key format. The two functions are implemented to perform this operation, one for the private key and the other for the public key.
...
([Appendix 4.3.2.1.4](#43214-key_expirepy)).

The AES keys stored in the HSM need to be rotated periodically in order to be cryptographically secure. In the event that access to the AES key is gained, the data still cannot be decrypted. This function implements this.
...
The user's record from the record's database (see [2.3](#23-record-management)) is fetched and the cipher text is decrypted to plaintext.
...
Key rotation is not required for RSA keys. This is because each time the user logins, both their RSA keys are refreshed.

## 2.3 Record management

In this system, data is organised into records. Users from the various clinical services interacting with the system, have the capability to store and retrieve records.

### 2.3.1 Encryption

([Appendix 4.3.3.1](#4331-encryptionpy)).
...
The AES key length should be 32 bytes. This also gives assurance that the AES key is only generated by the crypto system and no external keys are being used. If data is encrypted with an invalid key, the cipher text is corrupted and cannot be recovered to plaintext.

An initialisation vector, a random binary string of 16 bytes (or 128 bits), helps to ensure cryptographic randomness in the cipher texts.

A new cryptographic cipher is created, which is an object that can perform the encryption.
...
The encryptor creates the cipher text by encrypting each character in its internal buffer.

The final cipher text is returned as a concatenation of the initialisation vector and the newly generated cipher text. This will be required in when decrypting (see [2.3.2](#232-decryption)).

### 2.3.2 Decryption

([Appendix 4.3.3.2](#4332-decryptionpy)).
...
In [2.3.1](#231-encryption) the final cipher text was a combination of the initialisation vector and the cipher text components. String splicing splits it into its constituent components.

The user's AES key is checked to see if its of the correct length, and also verifies that only keys issued by the crypto system are used.
...
Finally, the `decryptor` returns the plain text in bytes, and this is converted to a string to be read.

### 2.3.3 Role Based Access Control (RBAC)

([Appendix 4.3.3.3](#4333-role_checkpy)).
...
([Appendix 4.3.3.4](#4334-record_storepy)).
...
- **`individual_access`**: if specific users are allocated access (see [2.5.1](#2351-individual-access-record-retrieval)).
...
A new JSON document is made is stored in the records database.

### 2.3.5 Retrieve records

([Appendix 4.3.3.5](#4335-record_retrievepy)).
...
The owner of the record has their corresponding AES key retrieved and the record is decrypted.
...
([Appendix 4.3.4.1](#4341-decrypt_datapy)).

The RSA data decryption process needs to be called every time a record store or access request is sent to the server side.
...
The RSA HSM is searched to find the PEM string of the record owner.
...
Highly efficient – ideal for data of large volumes without much of an impact on performance. AES can optimally use the resources its been allocated. It is suitable for a healthcare provider since they will have a lot of data traversing in the clinic. Key size is also sufficiently large enough that brute force attacks are prevented, so data remains protected.

### 3.1.2 RSA

Since data transmission may occur over unsecured networks, it is important that confidentiality is ensured at all times. In [2.1.2](#212-user-login), the public key is not secured at all and widely distributed to the client side but the private key isn't. This ensures that data encrypted with the public key can only be decrypted with the corresponding private key.

## 3.2 Compliance standards

The following compliance standards the solution complies with:
...
By encrypting data at both rest and transit reaffirms the "security by design" principle highlighted in GDPR and CCPA. It meets requirements such as Article 32 of GDPR [@intersoftconsulting1] and Section 1798.100 of CCPA [@casetext1].

### 3.2.2 Access control

The role-based access control system (RBAC) strengthens data protection by only allowing access to certain individuals based on their roles and permissions. This is to adhere to the principle of data "minimisation" outlined in Article 5 of GDPR [@intersoftconsulting2] and Section 1798.110 of CCPA [@casetext2].
